(function () {
	...
	'use strict';
	var colAbbr = 'crc'; // set separately for each college
	var libchatHash = 'a24b10a3580f241dc2aaf29a0b97ab2f'; // set separately for each college
	...
	app.controller('exploreFooterAfterController', ['$window', function($window) {
		...
		vm.checkForContent = function () {
			...
			if (content.length > 0) {
				...
				console.log(h);
				if ((h > 200) || (angular.element(document.querySelector('prm-browse-search')).length > 0)) {
					return true;
				}
			}
		};
